function getPrint() { try { return require('expo-print'); } catch(e) { return null; } }
function getSharing() { try { return require('expo-sharing'); } catch(e) { return null; } }
function getFileSystem() { try { return require('expo-file-system'); } catch(e) { return null; } }
...
export async function downloadPdfToDevice(html, filename) {
  const Print = getPrint(); if (!Print) throw new Error('PDF Print not available'); const { uri } = await Print.printToFileAsync({ html, base64: false });
  const safeFilename = sanitizeFilename(filename).endsWith('.pdf') ? sanitizeFilename(filename) : `${sanitizeFilename(filename)}.pdf`;

  const FileSystem = getFileSystem(); if (FileSystem?.StorageAccessFramework) {
    try {
  ...
  const Sharing = getSharing(); if (Sharing && await Sharing.isAvailableAsync()) {
