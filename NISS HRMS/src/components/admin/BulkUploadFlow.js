...
export function BulkUploadFlow({ type, title, getColumns, uploadFn, renderExtraFields, extra, templateFilename, fileFieldName = 'file' }) {
  ...
  const pickFile = async () => {
    try {
      ...
      const DocumentPicker = getDocumentPicker(); if (!DocumentPicker) return; const res = await DocumentPicker.getDocumentAsync({ type: SPREADSHEET_MIME_TYPES, copyToCacheDirectory: true });
