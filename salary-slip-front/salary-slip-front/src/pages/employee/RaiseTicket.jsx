 ...
 *
 * Company, unit and department are deliberately not fields: the server takes
 * them from the signed-in employee, so showing them as inputs would imply a
 * choice that does not exist (and that the API would ignore). They are shown
 * read-only instead, so the employee can see where the ticket is going.
 */
export default function RaiseTicket() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res?.status) {
        // The number is the thing the employee will quote later, so it goes in
        // the confirmation rather than a generic "submitted".
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-5 p-2 lg:p-6">
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Category" required>
            ...
            {selectedCategory?.description && (
              <p className="mt-1 text-xs text-gray-400">{selectedCategory.description}</p>
            )}
          </Field>
        ...
        {/* Read-only, for the reason given at the top of this file. */}
