...
export default function AttendanceUpload() {
  ...
  const [templateType, setTemplateType] = useState("monthly"); // "monthly" | "daily"
  const [templateDay, setTemplateDay] = useState(String(new Date().getDate()));
  ...
  const downloadTemplate = () => {
    ...
    if (templateType === "monthly") {
      headers = ["Employee Code", "Employee Name", ...dayList.map(String)];
      rows = employees.map((emp) => [emp.emp_code ?? "", emp.name ?? "", ...dayList.map(() => "")]);
      XLSX.writeFile(XLSX.utils.book_append_sheet(XLSX.utils.book_new(), XLSX.utils.aoa_to_sheet([headers, ...rows]), "Attendance"), `${selectedCompanyId}_attendance_monthly_${monthLabel}_${selectedYear}.xlsx`);
    } else {
      headers = ["Employee Code", "Employee Name", String(templateDay)];
      rows = employees.map((emp) => [emp.emp_code ?? "", emp.name ?? "", ""]);
      XLSX.writeFile(XLSX.utils.book_append_sheet(XLSX.utils.book_new(), XLSX.utils.aoa_to_sheet([headers, ...rows]), "Attendance"), `${selectedCompanyId}_attendance_daily_${templateDay}_${monthLabel}_${selectedYear}.xlsx`);
    }
    ...
    <div className="flex flex-col xl:flex-row gap-6">
      ...
      <div className="flex-1 flex flex-col gap-6 min-w-0">
        <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5 dark:border-white/10 dark:bg-white/[0.03]">
          ...
          <div className="mt-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-t border-gray-200 dark:border-white/10 pt-4">
              ...
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    name="templateType"
                    value="monthly"
                    checked={templateType === "monthly"}
                    onChange={(e) => setTemplateType(e.target.value)}
                    className="h-4 w-4 text-brand-600 focus:ring-brand-500 border-gray-300"
                  />
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Monthly Sheet</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name="templateType"
                    value="daily"
                    checked={templateType === "daily"}
                    onChange={(e) => setTemplateType(e.target.value)}
                    className="h-4 w-4 text-brand-600 focus:ring-brand-500 border-gray-300"
                  />
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Daily Sheet</span>
                </label>
              </div>
              ...
              {templateType === "daily" && (
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-sm text-gray-900 outline-none transition focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 dark:border-white/10 dark:bg-slate-950/60 dark:text-white"
                  >
                    {dayList.map((d) => (
            ...
            >
              Download {templateType === "monthly" ? "Monthly" : "Daily"} Template
            </Button>
          </div>
        </div>
