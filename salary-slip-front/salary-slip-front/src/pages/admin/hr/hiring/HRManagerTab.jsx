...
export default function HRManagerTab({ departments = [], people = [], openRequisitionForm, isHrManagerView = false }) {
    ...
    openRequisitionForm(r.id, `HR Review — ${r.title}`, (
      <div className="flex justify-end gap-2">
        ...
        <Button onClick={() => {
          ...
          hrApi.getRequisitionApprovalOptions(r.id, user?.accessToken, user?.tokenType, { company_code: companyScope?.companyId, unit: companyScope?.unit })
            .then(res => { if (res.status) setEligibleDirectors(res.data?.directors || []); })
  ...
  const handleForwardToDirector = async () => {
    if (!directorId) return toast.error("Select a director");
    try {
      const res = await hrApi.hrManagerForward(reviewReqId, { director_id: directorId, comment: reviewComment }, user?.accessToken, user?.tokenType);
      if (res.status) {
        toast.success("Forwarded to Director");
        setForwardModalOpen(false);
        setReviewReqId(null);
        openRequisitionForm(null); // close modal
        load();
      }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* HR Manager Review Modal */}
      <HrManagerReviewModal
        isOpen={Boolean(reviewReq)}
