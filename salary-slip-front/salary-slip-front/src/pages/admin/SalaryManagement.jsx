...
const ALL_SALARY_COLUMNS = [
  ...
  { field: "eduA", label: "Edu. A." },
  { field: "owa", label: "O.W.A." },
  { field: "ppa", label: "P.P.A" },
  { field: "pda", label: "P.D.A" },
  { field: "medA", label: "Med. A." },
  { field: "bonus", label: "Bonus" },
  { field: "lta", label: "L.T.A" },
  { field: "ha", label: "H.A" },
  { field: "mobA", label: "Mob. A." },
  { field: "productIncentive", label: "Product Incentive" },
...
function mapRecord(item) {
  ...
  return {
    ...
    eduA: Number(item.edu_a ?? 0),
    owa: Number(item.owa ?? 0),
    ppa: Number(item.ppa ?? 0),
    pda: Number(item.pda ?? 0),
    medA: Number(item.med_a ?? 0),
    bonus: Number(item.bonus ?? 0),
    lta: Number(item.lta ?? 0),
    ha: Number(item.ha ?? 0),
    mobA: Number(item.mob_a ?? 0),
    productIncentive: Number(item.product_incentive ?? 0),
...
export default function SalaryManagement() {
    ...
    async function loadDetail() {
      ...
      try {
        ...
        setDetail({
          ...
          comm: Number(item.comm ?? 0),
          other: Number(item.other ?? 0),
          edu_a: Number(item.edu_a ?? 0),
          owa: Number(item.owa ?? 0),
          ppa: Number(item.ppa ?? 0),
          pda: Number(item.pda ?? 0),
          med_a: Number(item.med_a ?? 0),
          mob_a: Number(item.mob_a ?? 0),
  ...
  const columnDefs = useMemo(() => {
    ...
    return [
      ...
      currencyColumn("eduA", "Edu. A.", { hide: isMobile || !visibleColumns.includes("eduA") }),
      currencyColumn("owa", "O.W.A.", { hide: isMobile || !visibleColumns.includes("owa") }),
      currencyColumn("ppa", "P.P.A", { hide: isMobile || !visibleColumns.includes("ppa") }),
      currencyColumn("pda", "P.D.A", { hide: isMobile || !visibleColumns.includes("pda") }),
      currencyColumn("medA", "Med. A.", { hide: isMobile || !visibleColumns.includes("medA") }),
      currencyColumn("bonus", "Bonus", { hide: isMobile || !visibleColumns.includes("bonus") }),
      currencyColumn("lta", "L.T.A", { hide: isMobile || !visibleColumns.includes("lta") }),
      currencyColumn("ha", "H.A", { hide: isMobile || !visibleColumns.includes("ha") }),
      currencyColumn("mobA", "Mob. A.", { hide: isMobile || !visibleColumns.includes("mobA") }),
      currencyColumn("productIncentive", "Product Incentive", { hide: isMobile || !visibleColumns.includes("productIncentive") }),
      ...
      currencyColumn("lwf", "LWF", {
        hide: isMobile || !visibleColumns.includes("lwf"),
        cellClass: "salary-ag-cell text-red-500",
      }),
