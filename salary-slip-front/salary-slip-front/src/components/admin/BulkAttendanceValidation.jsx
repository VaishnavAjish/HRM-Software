...
export default function BulkAttendanceValidation({
  headers,
  rawRows,
  onConfirm,
  onCancel,
  ...
  // A day column is any header that's a bare 1-31 number — that's what the
  // downloaded template uses, one column per day of the selected month.
  const dayColIdxs = useMemo(
    () => headers.reduce((acc, h, idx) => {
      const n = parseInt(String(h).trim(), 10);
      if (String(h).trim() !== "" && !Number.isNaN(n) && n >= 1 && n <= 31) acc.push(idx);
      return acc;
    }, []),
    [headers],
  );
            ...
            <thead className="sticky top-0 z-10">
              <tr className="bg-gray-100 dark:bg-gray-800">
                ...
                {headers.map((header, colIdx) => {
                  const isDayCol = dayColIdxs.includes(colIdx);
                  return (
                    <th key={colIdx} className="px-2 py-3 font-semibold text-gray-500 dark:text-gray-400 border-b border-r border-gray-200 dark:border-gray-700 whitespace-nowrap text-center" style={{ width: isDayCol ? 52 : 150 }}>
                ...
                return (
                  <tr key={row.id} className={
                    ...
                    {headers.map((_, colIdx) => {
                      ...
                      const width = isDayCol ? 52 : 150;
                      return (
                        <td key={colIdx} className={`px-2 py-2.5 border-b border-r border-gray-100 dark:border-gray-800 overflow-hidden ${isDayCol ? "text-center" : ""} ${fieldHasError ? "text-red-600 dark:text-red-400" : "text-gray-700 dark:text-gray-300"}`} style={{ width, maxWidth: width }}>
                          {isEditing(row.id, colIdx) ? (
                            <input
                              ...
                              className={`block w-full min-w-0 box-border rounded border-2 border-brand-500 bg-white px-2 py-1 text-xs text-gray-900 outline-none dark:bg-gray-800 dark:text-white ${isDayCol ? "text-center uppercase" : ""}`}
                              maxLength={isDayCol ? 1 : undefined}
                              autoFocus
