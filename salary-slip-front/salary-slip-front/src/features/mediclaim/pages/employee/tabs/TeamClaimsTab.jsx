...
export default function TeamClaimsTab() {
  ...
  useEffect(() => {
    ...
    mediclaimApi.teamClaims({ page, perPage: PER_PAGE }, user.accessToken, user.tokenType)
      .then((res) => {
        if (cancelled) return;
      ...
      .catch((err) => {
        if (cancelled) return;
        setState({ loading: false, rows: [], total: 0, error: err?.message || "Failed to load team claims." });
      });
